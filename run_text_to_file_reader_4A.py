...
def read_aridialect_sentences_thorsten(model_id, device):
    tts = tts_dict[model_id](device=device, speaker_embedding="default_speaker_embedding.pt")

    path="/mnt/1E4AD19F4AD173CD/Christoph/uni/speech_synthesis"
    with open(os.path.join(path, "aridialect_spo/test-text-spo.txt"), "r", encoding="utf8") as f:
        sents = f.read().split("\n")
    output_dir = "audios/test_{}".format(model_id)
    if not os.path.isdir(output_dir):
        os.makedirs(output_dir)
    for index, sent in enumerate(sents):
        print(sent)
        sentid= sent.split("|")[0]
        wavname=os.path.join(path, "aridialect_spo/aridialect_wav16000", sentid+".wav")
        tts.read_to_file(text_list=[sent.split("|")[1]], file_location=output_dir + "/{}.wav".format(sentid))
