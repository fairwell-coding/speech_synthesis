...
from Preprocessing.phonemes import create_phonemes
...
class TextFrontend:
    ...
    def __init__(self,
                 ...
                 #path_to_phoneme_list="Preprocessing/ipa_phonemes",
                 path_to_phoneme_list="Preprocessing/ipa_phonemes_complete.txt",
                 silent=True,
                 allow_unknown=False,
                 inference=False,
                 strip_silence=True):
        ...
        if language == "en":
            ...
            self.expand_abbreviations = lambda x: x
            if not silent:
                print("Created an Austrian Text-Frontend")

        else:
            print("Language not supported yet")
            sys.exit()

    def string_to_tensor(self, text, path_to_wavfile, view=False):
        """
        ...
        layer
        """
        phones = self.get_phone_string(text=text, path_to_wavfile=path_to_wavfile,  include_eos_symbol=False)
        if view:
            print("Phonemes: \n{}\n".format(phones))
        phones_vector = list()
    ...
    def get_phone_string(self, text, path_to_wavfile, include_eos_symbol=True):
        ...
        path_to_lab_dir = path_to_wavfile.replace("wav16000","labels")
        path_to_labfile = path_to_lab_dir.replace(".wav", ".lab")
        # print(path_to_labfile)
        ...
        phones = create_phonemes(path_to_labfile)
